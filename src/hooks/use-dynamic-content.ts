...
import { getProgress, saveProgress } from '@/lib/progress-store';
import {
  getDailyDiscovery,
...
export function useDynamicContent(userLevel: number = 1) {
  ...
  const learnWords = useCallback((wordIds: string[]) => {
    ...
    const newWordIds = [...new Set(wordIds)].filter(id => !learnedIds.has(id));
    if (newWordIds.length === 0) return;

    markWordsAsLearned(newWordIds);
    const progress = getProgress();
    saveProgress({
      completedWords: [...new Set([...progress.completedWords, ...newWordIds])],
      xp: progress.xp + newWordIds.length * 10,
    });
    ...
    setContent(prev => ({
      ...
      newWords: prev.newWords.map(w => 
        newWordIds.includes(w.id) ? { ...w, isNew: false } : w
      ),
    }));
    ...
    setStats(prev => ({
      ...prev,
      totalLearned: prev.totalLearned + newWordIds.length,
    }));
  }, []);
