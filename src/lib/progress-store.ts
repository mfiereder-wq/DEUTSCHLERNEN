...
export const PROGRESS_UPDATED_EVENT = 'deutschlernen-progress-updated';
...
function normalizeProgress(value: Partial<UserProgress>): UserProgress {
  ...
  return {
    ...
    xp: typeof value.xp === 'number' && Number.isFinite(value.xp) && value.xp >= 0
      ? value.xp
      : defaults.xp,
    completedWords: Array.isArray(value.completedWords) ? value.completedWords : defaults.completedWords,
    completedSentences: Array.isArray(value.completedSentences) ? value.completedSentences : defaults.completedSentences,
    completedQuizzes: Array.isArray(value.completedQuizzes) ? value.completedQuizzes : defaults.completedQuizzes,
    currentLevel: typeof value.currentLevel === 'number' ? value.currentLevel : defaults.currentLevel,
    streak: typeof value.streak === 'number' ? value.streak : defaults.streak,
    lastActive: typeof value.lastActive === 'string' ? value.lastActive : defaults.lastActive,
  };
}
...
export function saveProgress(progress: Partial<UserProgress>): void {
  ...
  try {
    ...
    const updated = normalizeProgress({ ...current, ...progress });
