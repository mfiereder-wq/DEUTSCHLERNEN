...
export function getStreak(): number {
  ...
  let parsed: { streak?: number; lastActiveDate?: string };
  try {
    parsed = JSON.parse(streakData);
  } catch {
    return 0;
  }
...
export function updateStreak(): number {
  ...
  const streakData = localStorage.getItem(STREAK_KEY);
  let parsed: { streak: number; lastActiveDate: string | null } = { streak: 0, lastActiveDate: null };
  if (streakData) {
    try {
      parsed = { ...parsed, ...JSON.parse(streakData) };
    } catch {
  ...
  if (isNewDay()) {
    const lastActive = parsed.lastActiveDate ? new Date(parsed.lastActiveDate) : null;
